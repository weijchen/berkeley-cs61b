...
public class WorldGeneratorFinal {
    ...
    /* Initial code caller */
    public static Position[][] addHexagon(TETile[][] world) {
    ...
    public static Position fetchPlayer(Position[][] posMap) {
        for (Position[] pRow : posMap) {
            for (Position p : pRow) {
                if (p.getTType() == Tileset.PLAYER) {
                    return p;
                }
            }
        }
    ...
    public static Position addPlayer(Position[][] posMap) {
        ArrayList<Position> candidate = new ArrayList<>();
        for (Position[] pRow : posMap) {
            for (Position p : pRow) {
                if (p.getTType() == Tileset.FLOOR) candidate.add(p);
            }
        }
        int canID = (int) (Math.random()*candidate.size());
        candidate.get(canID).setTType(Tileset.PLAYER);
        return candidate.get(canID);
    }
...
//                    int wallCount = 0;
                    for (Position n : neighbor) {
                        if (n.getTType() == Tileset.FLOOR) pass = true;
//                        if (n.getTType() == Tileset.WALL) wallCount += 1;
                    }
                    if (pass) candidate.add(p);
                }
            }
        }
        int canID = (int) (Math.random()*candidate.size());
